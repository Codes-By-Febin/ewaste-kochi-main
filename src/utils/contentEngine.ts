...
class ContentEngine {
  private content: LinkingContext;

  constructor(content: LinkingContext) {
    this.content = content;
  }

  // Generate category pages for services
  generateServiceCategories(): GeneratedPage[] {
  ...
  generateLocationHubs(): GeneratedPage[] {
  ...
  generateBlogTagPages(): GeneratedPage[] {
  ...
  generateInternalLinks(content: string, currentSlug: string, contentType: 'service' | 'blog' | 'location'): string {
  ...
  generateRelatedContent(currentItem: ContentItem, contentType: 'service' | 'blog' | 'location', limit: number = 3) {
    const related: Array<{ type: string; item: any; score: number }> = [];
  ...
  private isKeywordMatch(keyword: string, text: string): boolean {
  ...
  private generateCategoryPageContent(category: string, items: any[], type: string): string {
  ...
  private generateLocationHubContent(district: string, locations: any[], services: any[]): string {
  ...
  private generateTagPageContent(tag: string, posts: any[]): string {
  ...
  private generateCategorySchema(category: string, items: any[], type: string): any {
    return {
      "@type": "CollectionPage",
      ...
      "mainEntity": {
        ...
        "itemListElement": items.map((item, index) => ({
          ...
          "item": {
            ...
            "url": `https://ewastekochi.com/${type}s/${item.slug}/`
          }
        }))
      }
    };
  ...
  private generateLocationHubSchema(district: string, locations: any[]): any {
    return {
      "@type": "CollectionPage",
  ...
  private generateBlogSchema(posts: any[]): any {
    return {
      "@type": "CollectionPage",
...
export { ContentEngine, type GeneratedPage, type LinkingContext };
